...
var userAgent = navigator.userAgent.toLowerCase(),
  ...
  plugins = {
    ...
    higCharts: { charts: $(".higchart"), legend: $(".chart-legend") },
    d3Charts: $(".d3-chart"),
      ...
      var bar = $(plugins.progressBar[i]),
        initProgress = function() {
          ...
          if (!bar.hasClass("animated-first") && isScrolledIntoView(bar)) {
            ...
            bar
              .find(".progress-value")
              .countTo({ refreshInterval: 40, from: 0, to: end, speed: 500 });
  ...
  if (plugins.swiper.length) {
    plugins.swiper.each(function() {
      ...
      slider
        ...
        .swiper({
          autoplay:
            !isNoviBuilder && $.isNumeric(slider.attr("data-autoplay"))
          ...
          keyboardControl: !isNoviBuilder
            ? slider.attr("data-keyboard") === "true"
            : false,
          mousewheelControl: !isNoviBuilder
            ? slider.attr("data-mousewheel") === "true"
            : false,
          mousewheelReleaseOnEdges:
            slider.attr("data-mousewheel-release") === "true",
          ...
          onInit: function(swiper) {
            ...
            if (!isNoviBuilder) toggleSwiperInnerVideos(swiper);
            if (!isNoviBuilder) toggleSwiperCaptionAnimation(swiper);
            slider.find(".swiper-parallax").each(function() {
              var $this = $(this),
                speed;
          ...
          for (var j = 0; j < section.length; j++) {
            $(section[section.length - j - 1]).append(
              '<span class="countdown-letter">' +
                settings.format[settings.format.length - j - 1] +
                "</span>"
            );
          }
        };
  ...
  if (plugins.checkoutRDTabs.length) {
    ...
    for (i = 0; i < plugins.checkoutRDTabs.length; i++) {
      ...
      checkoutTab.RDMaterialTabs({
        ...
        marginContent: 10,
        margin: 0,
        responsive: { 480: { items: 2 }, 768: { dragList: false, items: 3 } },
        callbacks: {
  ...
  if (plugins.higCharts.charts.length) {
    ...
    for (i = 0; i < plugins.higCharts.charts.length; i++) {
      var higchartsItem = $(plugins.higCharts.charts[i]),
        higChartsItemObject = parseJSONObject(
          higchartsItem,
          "data-graph-object"
        );
      if (
        !higchartsItem.attr("data-parent-chart") &&
        !higchartsItem.attr("data-child-chart")
      ) {
        higchartsItem.highcharts(higChartsItemObject);
      } else {
        if (higchartsItem.attr("data-child-chart")) {
          var childGraph = higchartsItem.attr("data-child-chart"),
            higChartsChildObject = parseJSONObject(
              childGraph,
              "data-graph-object"
            );
          masterChart = higchartsItem
            .highcharts(higChartsItemObject, function() {
              detailChart = $(childGraph)
                .highcharts(higChartsChildObject)
                .highcharts();
            })
            .highcharts();
        }
  ...
  if (plugins.higCharts.legend.length) {
    ...
    for (i = 0; i < plugins.higCharts.legend.length; i++) {
      ...
      for (j = 0; j < legendItems.length; j++) {
        ...
        legendObj = { legendItem: legendItem, legend: legend };
        legendItem.on(
          "click",
          $.proxy(function(e) {
            var _this = this;
  ...
  if (plugins.flotCharts.length) {
    ...
    for (i = 0; i < plugins.flotCharts.length; i++) {
      var flotChartsItem = plugins.flotCharts[i],
        flotChartItemObject = parseJSONObject(
          flotChartsItem,
          "data-graph-object"
        ),
      ...
      callbacks: {
        onInit: function() {
          ...
          if ($html.hasClass("desktop")) {
            makeVisible(
              plugins.galleryRDTabs.find(".rd-material-tab:first-child .image")
            );
          }
        },
        ...
        onChangeEnd: function() {
          if ($html.hasClass("desktop")) {
            makeVisible(
              plugins.galleryRDTabs.find(".rd-material-tab-active .image")
            );
          }
        }
      }
